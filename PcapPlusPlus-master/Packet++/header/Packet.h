...
namespace pcpp
{
	...
	/**
	 * Packet is middleware between RawPacket and Layer
	 * @class Packet
	...
	public:
		...
		/**
		 * 从开辟内存的RawPacket上构建一个Packet，使用指针保存RawPacket，不会进行数据拷贝
		...
		/**
		 * 设置 RawPacket 并且重新创建所有的 layers
		 * Set a RawPacket and re-construct all packet layers 
		 * @param[in] rawPacket Raw packet to set
		 * @param[in] freeRawPacket A flag indicating if the destructor should also call the raw packet destructor or not 判断是否要自己释放
