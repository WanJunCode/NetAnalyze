...
#pragma pack(push,1)
	struct tcphdr {
		...
		uint16_t reserved:4,
		/** Specifies the size of the TCP header in 32-bit（4*8） words 指出TCP头的长度*/
		dataOffset:4,
		/** FIN flag */
		finFlag:1,
		/** SYN flag */
...
#define PCPP_TCPOLEN_EXP_MIN        2
	...
	public:
		...
		/**
		 * 用于描述 NOP 或 NOP 的枚举类型
		 * An enum to describe NOP and NOP TCP options. Used in one of this class's c'tors
		 */
	...
	private:
		...
		TLVRecordReader<TcpOption> m_OptionReader;				// 选项option读取器，管理tcp选项
