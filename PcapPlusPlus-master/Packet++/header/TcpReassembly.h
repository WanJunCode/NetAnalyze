 ...
 * 一个pcpp#TcpReassembly实例管理多个tcp连接，支持tcp重传、失序数据包、丢失数据包。支持回调函数
 ...
 * - If the packet data matches these criteria原则 a callback is being invoked. This callback is supplied by the user in the creation of the pcpp#TcpReassembly instance. 
 * - This callback contains the new data (of course), but also information about the connection (5-tuple, 4-byte hash key describing the connection, etc.) 
 * - and also a pointer to a "user cookie", meaning a pointer to a structure provided by the user during the creation of the pcpp#TcpReassembly instance
 * - If the data in this packet isn't new, it's being ignored
 * - If the data in this packet isn't expected (meaning this packet came out-of-order), then the data is being queued internally内部排列 and will be sent to the user when 
 * - its turn arrives
 *   (meaning, after the data before arrives)
 * - If the missing data doesn't arrive until a new message from the other side of the connection arrives or until the connection ends - this will be considered as missing 
 * - data and the queued data will be sent to the user, but the string "[X bytes missing]" will be added to the message sent in the callback
 * - pcpp#TcpReassembly supports 2 more callbacks - one is invoked when a new TCP connection is first seen and the other when it's ended (either by a FIN/RST packet or 
 * - manually by the user).
 *   Both of these callbacks contain data about the connection (5-tuple, 4-byte hash key describing the connection, etc.) and also a pointer to a "user cookie", 
 * - meaning a pointer to a
 *   structure provided by the user during the creation of the pcpp#TcpReassembly instance. The end connection callback also provides the reason for closing it 
 ...
 * When the connection is closed the information is not being deleted from memory immediately. There is a delay between these moments. Existence of this delay is 
 ...
 * - the user can use the information about connections managed by pcpp#TcpReassembly instance. Following methods are used for this purpose: 
 *   pcpp#TcpReassembly#getConnectionInformation and pcpp#TcpReassembly#isConnectionOpen.
 * 
 ...
 * - pcpp#TcpReassemblyConfiguration#maxNumToClean - to avoid performance overhead(过载) when the cleanup is being performed, this parameter is used. 
 *   It defines the maximum number of items to be removed per one call of pcpp#TcpReassembly#purgeClosedConnections
 *
 */
