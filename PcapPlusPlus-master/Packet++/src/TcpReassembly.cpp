...
void TcpReassembly::reassemblePacket(Packet& tcpData)
{
	...
	// set the TCP payload size 获得tcp有效载荷大小
	...
	// calculate flow key for this packet  根据端口大小进行交换
	...
	if (iter == m_ConnectionList.end())		// 新的连接
	{
		...
		m_ConnectionInfo[flowKey] = tcpReassemblyData->connData;		// ip port time flowkey

		// fire connection start callback 执行回调函数
		if (m_OnConnStart != NULL)
			m_OnConnStart(tcpReassemblyData->connData, m_UserCookie);
	}
	else // connection already exists 更新连接的时间
	{
		...
		{
			if (currTime.tv_usec > tcpReassemblyData->connData.endTime.tv_usec)		// 比较微秒
			{
				tcpReassemblyData->connData.setEndTime(currTime);
			}
		}
	...
	int sideIndex = -1;
	bool first = false;			// 判断是否是会话中的第一个报文
	...
	// if there are already 2 sides open for this connection  如果会话两端都有数据包
	else if (tcpReassemblyData->numOfSides == 2)
	{
		// 通过判断 ip 和 port 确认数据包方向
	...
	// If this is the case then treat the out-of-order packet list as missing data and send them to the user (callback) together with an indication that some data 
	// was missing.
	...
	// For example: let's assume these are HTTP messages. If we're seeing the first packet of a response this means the server has already received the full request
	// and is now starting to send the response. So if we still have out-of-order packets from the request it probably means that some packets were lost during 
	// the capture. So we don't expect the client to continue sending packets of the previous request, so we'll treat the out-of-order packets as missing data
	//
	// I'm aware that there are edge cases where the situation I described above is not true, but at some point we must clean the out-of-order packet list to avoid 
	// memory leak.I decided to do what Wireshark does and clean this list when starting to see a message from the other side
	...
	{
		// 接收到新方向的数据包，但是上一个方向还有数据，有可能上一个方向的数据包丢失了
	...
	tcpReassemblyData->prevSide = sideIndex;	// 记录上一个数据包的方向

	// extract sequence value from packet  网络字节序转主机字节序
	...
	if (first)
	{
		...
		if (tcpLayer->getTcpHeader()->synFlag != 0)
			tcpReassemblyData->twoSides[sideIndex].sequence++;		// 如果是 syn 的数据包，下一个seq应该+1
	...
	// if packet sequence is smaller than expected - this means that part or all of the TCP data is being re-transmitted 重传数据包
	if (sequence < tcpReassemblyData->twoSides[sideIndex].sequence)
	{
		...
		// this means that some of payload is new 有部分数据是新的
	...
	else if (sequence == tcpReassemblyData->twoSides[sideIndex].sequence)
	{
		...
		// now that we've seen new data, go over the list of out-of-order packets and see if one or more of them fits now  获得新的数据后从out-of-order数据中检查是否有合适的增加
...
void TcpReassembly::handleFinOrRst(TcpReassemblyData* tcpReassemblyData, int sideIndex, uint32_t flowKey)
{
	...
	// 判断两边是否都收到 fin or rst， 如果只是一边收到则删除当前side
