...
void Packet::setRawPacket(RawPacket* rawPacket, bool freeRawPacket, ProtocolType parseUntil, OsiModelLayer parseUntilLayer)
{
	...
	while (curLayer != NULL && (curLayer->getProtocol() & parseUntil) == 0 && curLayer->getOsiModelLayer() <= parseUntilLayer)
	{
		...
		curLayer->parseNextLayer();									// 解析下一层协议
...
// freeRawPacket   = False
// parseUntil      = UnknownProtocol				解析到指定协议
// parseUntilLayer = OsiModelLayerUnknown			解析到指定OSI层次
...
void Packet::destructPacketData()
{
	...
	{
		delete m_RawPacket;			// 负责内部 生数据包的清理
	}
}
...
void Packet::reallocateRawData(size_t newSize)
{
	...
	// set the new array to RawPacket 内部使用 new []
	...
	// 遍历layer，重新设置数据指针。每个layer的数据指针都指向 dataPtr 内存块的指定位置
	Layer* curLayer = m_FirstLayer;
	while (curLayer != NULL)
	{
...
bool Packet::extendLayer(Layer* layer, int offsetInLayer, size_t numOfBytesToExtend)
{
	...
	// verify layer is allocated to this packet 明确 Packet 与 Layer 之间的关系
	...
	// Packet进行扩展的策略 *2 , 如果大于2倍的长度则使用该长度
...
bool Packet::shortenLayer(Layer* layer, int offsetInLayer, size_t numOfBytesToShorten)
{
	...
	// remove data from raw packet   将当前layer的偏移offset转换到rawPacket的数据指针上
	...
	// go over all layers from the first layer to the last layer and set the data ptr and data length for each layer
	Layer* curLayer = m_FirstLayer;
	bool passedExtendedLayer = false;	// 判断是否经过了extent的layer
	while (curLayer != NULL)
	{
		// set the data ptr
		curLayer->m_Data = (uint8_t*)dataPtr;
		...
		if (curLayer->getPrevLayer() == layer)	// 如果当前遍历到的layer前一层是extent的layer，表示已经pass
			passedExtendedLayer = true;
		...
		if (!passedExtendedLayer)
			curLayer->m_DataLen -= numOfBytesToShorten;	//在extent的layer之前的layer都需要减去相应的数据长度
