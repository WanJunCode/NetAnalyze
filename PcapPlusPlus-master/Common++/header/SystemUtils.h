...
namespace pcpp
{
	...
	struct SystemCore
	{
		/**
		 * 掩码，使用bit表示该core的位置
		 * Core position in a 32-bit mask. For each core this attribute holds a 4B (4*8=32) integer where only 1 bit is set, according to the core ID.
	...
	typedef uint32_t CoreMask;			// 定义 cpu掩码类型
	...
	private:
		EventHandlerCallback m_ApplicationInterruptedHandler;				// 中断信息处理器
		void* m_ApplicationInterruptedCookie;								// 中断信息处理器 参数

		// private c'tor 私有化构造函数，维护单例模式
		ApplicationEventHandler();

#ifdef WIN32
		static BOOL WINAPI handlerRoutine(DWORD fdwCtrlType);
#else
		pthread_mutex_t m_HandlerRoutineMutex;				// 锁，同步信号处理事件
