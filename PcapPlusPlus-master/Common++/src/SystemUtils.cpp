...
#else
	return sysconf(_SC_NPROCESSORS_ONLN);				// 获得系统 cpu 数量
#endif
}

CoreMask getCoreMaskForAllMachineCores()
{
	int numOfCores = getNumOfCores() < 32 ? getNumOfCores() : 32;			// cpu 数量不能超过 32
	CoreMask result = 0;
	for (int i = 0; i < numOfCores; i++)
	{
		result = result | SystemCores::IdToSystemCore[i].Mask;				// 并操作符
	}

	return result;
}

// 通过 core vector创建内核掩码
